...
FormSchema.methods.toJSONForSearch = function (user, option) {
  const returnObj = {};
  ['slug', 'title', 'description', 'createdAt', 'updatedAt', 'tags', 'enabled', 'public', 'type',
    'ofPayment'
  ].forEach((field) => {
  ...
  switch (option) {
    case 'filled':
      ['originalSlug', 'filledBy'].forEach((field) => {
        returnObj[field] = this[field];
      })
      break;
    case 'search':
      returnObj.liked = user ? user.isLike(this._id) : false;
      returnObj.author = this.author.toProfileJSONFor(user);
      ['likesCount', 'viewsCount', 'commentsEnabled', 'textType', 'documentType'].forEach(
        (field) => {
          returnObj[field] = this[field];
        })
      break;
